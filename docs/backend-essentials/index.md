...
短链接服务实现方式：哈希转换成固定长度字符串+自增id base64编码+生成数据库映射关系+重定向

Golang内存泄露：Goroutine无法退出导致其引用内存无法释放；文件句柄或网络连接未关闭；切片引用不当导致底层内存无法回收；map过大；循环引用；定时器未停止

* defer确保资源释放，context管理生命周期，pprof进行性能分析，代码审查保证质量
...
132模式检测：单调栈，从右往左遍历，third记录nums[k]最大可能值，大于栈顶弹栈更新third，小于third则返回true