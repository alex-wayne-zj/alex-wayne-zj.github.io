...
单机大量TIME_WAIT, CLOSE_WAIT连接：2MSL导致TCP连接占用端口无法被重用；服务端没有正确关闭socket
...
> 套接字的linger选项（停留时间，默认是立刻关闭套接字，未发送数据丢弃）
> 
> 套接字为什么要禁用Nagle算法（粘包算法，禁止便于立刻发送提高实时性）

从上到下：Socket库 - TCP/UDP + IP - 网卡驱动 + 网卡

常见网络协议

* WebSocket是一种全双工通信协议，允许客户端和服务器实时通信。长连接，应用于实时场景（在线聊天、游戏、股票）由 TCP 长连接升级而来，客户端可自定义并加密数据帧，通过定期心跳机制来保证长连接
...
* 常用Redis分布式锁：SETNX 原子操作设置过期时间（Set if not exist）；RedLock：为防止单点宕机问题，需要获得多数 Redis 节点锁才算真正获得锁（提高可靠性）
...
* RocketMQ：Alibaba开源的分布式消息队列系统，专注于高可用、高性能、事务。吞吐量略逊于Kafka，但可靠性更好，且为永久存储

> RabbitMQ：近20年前的产物，处理大流量和大并发时受限（kafka和rocketmq的qps都可以达到10w+），Exchange负责路由分发，Queue负责存储

Kafka支持主题数相对较少，只有百级别（Scala）；RocketMQ相比kafka性能弱一点（Java）
...
如何保证 Redis 缓存和 MySQL 的数据一致性

* Cache-aside，读时先读缓存，写时先写数据库，再更新或删除缓存中数据。短暂的数据不一致
* 延迟双删 + MQ 异步重试
