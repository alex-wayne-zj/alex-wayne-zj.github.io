...
动态规划之最长回文子序列：回文子串的两端性质是状态转移方程的关键；从短序列到长序列因此需要注意循环顺序

区间操作之最少箭射爆气球：sort.Slice排序；右边界判断
...
堆排序：heapify 递归交换父子建立大根堆；heap_sort 交换头尾+heapify维护堆
...
相等行列对：更快的遍历 i := range(n)

删除二叉搜索树节点：两子数都不为空时，右子树最小节点为新根节点，递归删除

K个一组翻转链表：第一组单独处理，中间要细致，结尾也需要单独处理
...
滑动窗口最大值：pair记录index和值，自上至下删除堆顶维护堆性质，自下而上添加新元素维护堆性质
...
螺旋矩阵：递归+边界条件；golang不可以直接切割二维切片
...
反转链表2：记录反转前后的节点，考虑边界情况

最小栈：存每个索引对应最小值的辅助单调栈

相交链表：同时遍历到头交换，n+m复杂度

比较版本号：strings.Split(), strconv.Atoi()
